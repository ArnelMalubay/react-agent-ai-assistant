...
def pdf_reader(filepath: str) -> str:
    """
    Read text content from a PDF file. Can be used for resumes, job postings, or any other document.
    The document will be stored in the agent's memory for future reference.
    ...
    try:
        ...
        return f"Successfully read document '{filename}'. Content preview:\n{clean_text(text_content)}"
...
def web_search(query_or_url: str) -> str:
    """
    Search for content online using Tavily search.
    Can be used to find job postings, company information, or any other web content.
    
    Args:
        query_or_url: Either a search query or URL to a webpage
        
    Returns:
        str: The search results content
    """
    try:
        # Use Tavily search for online content
        results = tavily_search.invoke(query_or_url)
        return f"Search results for '{query_or_url}':\n{results['answer']}"
    
    except Exception as e:
        return f"Error searching for '{query_or_url}': {str(e)}"


@tool
def list_documents() -> str:
    """
    List all available documents (resumes, job postings, etc.) in the agent's memory.
    
    Returns:
        str: Formatted list of available documents
    """
    return "This tool will list available documents. The agent should check its state for document data."


@tool
def list_searched_content() -> str:
    """
    List all available searched content (job postings, company info, etc.) in the agent's memory.
    
    Returns:
        str: Formatted list of available searched content
    """
    return "This tool will list available searched content. The agent should check its state for search data."
...
def custom_tool_node(state: AgentState):
    ...
    for tool_call in last_message.tool_calls:
        ...
        if tool_name == "pdf_reader":
            result = pdf_reader.invoke(tool_args)
        ...
        elif tool_name == "list_searched_content":
            if "job_summaries" in state and state["job_summaries"]:
                result = "Available searched content:\n" + "\n".join([f"• {query}" for query in state["job_summaries"].keys()])
            else:
                result = "No searched content available. Please search for content first."
            ...
            if "resumes" not in state or resume_filename not in state["resumes"]:
                ...
                job_content = state["job_summaries"][job_query]["content"]
